...
| **Fitbit** | code shipped, **platform being replaced** | `sync()` -> `fitbit_daily` works against the legacy Fitbit Web API, which Google shuts off in September 2026. The replacement is the Google Health API — an aggregation layer over a Google account, with Restricted scopes that need a privacy and security review. The connector has to be rewritten against it; the framework around it does not |
| **Strava** | code shipped, registerable today | `providers/strava.py`; `sync()` -> `strava_daily`: activity count, moving minutes, distance, elevation, Relative Effort, main-activity HR. Any free Strava account can register the app (see `docs/connectors-setup.md`); routes/GPS deliberately never stored. New apps are capped at 1 connected athlete until a limit bump is requested |
